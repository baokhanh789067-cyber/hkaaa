...
def prepare_model(args, logger):
    ...
    if args.optimizer_mode == "adapter":
        ...
        if 'vilt' in args.encoder_name:
            ...
            model.add_lora(rank=r, alpha=alpha)
        args.personal_params_names += ['lora_A_0', 'lora_B_0']
        args.shared_params_names = ['lora_A_1', 'lora_B_1']
        ...
        # 1. 先添加 LoRA 层 (使用 argparse 里的参数，如果没有就用默认值)
        r = getattr(args, 'lora_r', 8)
        alpha = getattr(args, 'lora_alpha', 16)
        model.add_lora(rank=r, alpha=alpha) 
...
def main():
    ...
    parser.add_argument("--adapter_config", default="pfeiffer", type=str, help="Adapter config to use")
    ...
    parser.add_argument("--wandb_freq", type=int, default=5,
    ...
    if args.ordered_cl_tasks == "scene":
        ...
        args.ordered_cl_tasks = ["art", "abstract", "toronto", "gqa"]
    ...
    if args.do_train:
        ...
        if args.do_single:
            ...
            for comm_round in range(args.comm_rounds):
                ...
                if True:  # [Paper Mode] Modified to evaluate every round
                    scores = 0.
